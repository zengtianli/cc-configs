...
替代已归档的 `/cf-dns` `/cf-audit`。后端：`~/Dev/tools/dev/lib/tools/cloud/cf_api.py` + `cloud/cf_audit.py`，凭证自动从 `~/.personal_env` 读取。
...
```bash
python3 ~/Dev/tools/dev/lib/tools/cloud/cf_audit.py
```
...
1. 所有调用直接执行 `python3 ~/Dev/tools/dev/lib/tools/cloud/cf_api.py <args>`，把 $ARGUMENTS 原样传过去
